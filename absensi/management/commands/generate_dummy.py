"""
Django management command untuk generate data dummy absensi tamu
Jalankan dengan: python manage.py generate_dummy
"""
...
class Command(BaseCommand):
    help = 'Generate data dummy absensi tamu dengan variasi instansi BUMN'
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=50,
            help='Jumlah data dummy yang akan di-generate (default: 50)',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'🚀 Generating {jumlah} data dummy...\n'))
        ...
        for i in range(jumlah):
            ...
            if (i + 1) % 10 == 0:
                self.stdout.write(f'  ✓ Generated {i + 1}/{jumlah} data...')

        self.stdout.write(self.style.SUCCESS(f'\n✅ Successfully generated {jumlah} dummy data!\n'))
        self.stdout.write(self.style.SUCCESS('Statistik:'))
        self.stdout.write(f'  - Total data: {AbsensiTamu.objects.count()}')
        
        # Count data hari ini
        start_of_day = timezone.make_aware(datetime.combine(hari_ini, datetime.min.time()))
        end_of_day = start_of_day + timedelta(days=1)
        data_hari_ini = AbsensiTamu.objects.filter(
            tanggal_waktu__gte=start_of_day,
            tanggal_waktu__lt=end_of_day
        ).count()
        self.stdout.write(f'  - Data hari ini: {data_hari_ini}')
        
        # Count data 7 hari terakhir
        start_week = timezone.make_aware(datetime.combine(hari_ini - timedelta(days=7), datetime.min.time()))
        data_week = AbsensiTamu.objects.filter(tanggal_waktu__gte=start_week).count()
        self.stdout.write(f'  - Data 7 hari terakhir: {data_week}\n')
        
        self.stdout.write(self.style.SUCCESS('🎉 Data dummy berhasil dibuat!'))
        self.stdout.write(self.style.SUCCESS('Silakan buka admin panel dan download PDF untuk melihat hasilnya.'))
